...
async def _check_ui_ready():
    result = await studio_ui_status()
    if not result.get('ready'):
        raise HTTPException(409, result.get('reason') or 'Chưa mở ô tạo video trên Flow')


@router.get("/api/studio/ui-status")
async def studio_ui_status():
    client = get_flow_client()
    if not client.connected:
        raise HTTPException(503, "Extension chưa kết nối")
    result = await client._send('ui_generate_video', {
        'projectId': config.FLOW_PROJECT_ID, 'mode': 'probe',
    }, timeout=30)
    if result.get('error'):
...
async def _run_advanced_fashion_pipeline(job_id: str, req: AdvancedFashionVideoRequest):
    ...
    try:
        ...
        for sc in (created_scenes[:1] if ui_mode else created_scenes):
        ...
        while True:
            ...
            progress_val = 55 + int((completed_count / len(created_scenes)) * 35)
            job["progress"] = progress_val
            job["message"] = f"Đã kết xuất {completed_count}/{len(created_scenes)} clip ({completed_count * 5}s / {req.duration_seconds}s)..."

            if completed_count == len(video_reqs):
                if len(video_reqs) == len(created_scenes):
                    break
                sc = created_scenes[len(video_reqs)]
                r = await crud.create_request(
                    req_type="GENERATE_VIDEO", scene_id=sc['id'], project_id=pid,
                    video_id=vid, orientation=req.orientation,
                )
                video_reqs.append(r['id'])
            if time.time() - start_wait > max(1800, len(created_scenes) * 660):
        ...
        for i, sc in enumerate(scenes_in_order):
            ...
            if trim_video(str(clip_path), str(trimmed_path), start=0.0, end=5.0):
                ...
                raise RuntimeError(f"Không cắt được cảnh {i+1} thành 5 giây; giữ clip gốc để xử lý lại")
  ...
  <script>
    ...
    function uploadAsset(file, imgElemId, promptViewId, previewViewId, mediaInputId) {
      ...
      reader.onload = async function(evt) {
        document.getElementById(mediaInputId).value = '';
        ...
        try {
          ...
          const data = await res.json();
          if (!res.ok) throw new Error(typeof data.detail === 'string' ? data.detail : 'Không tải được ảnh lên Flow');
          if (data.media_id) {
